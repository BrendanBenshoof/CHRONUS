...
We ran our experiments using Amazons's EC2 service.  Each node was an individual EC2 small instance \cite{amazon-instances} with a preconfigured Ubuntu 12.04 image.  These instances were capable enough that they could provide constant compuation, but still were incapable enough that they would be overwhelmed by traffic on occasions, creating a constant churn effect in the ring.  
...
\begin{figure}
    ...
    \caption{The projected speedup for different sized jobs. Note that as the jobs become larger, the speedup from ChordReduce becomes closer and closer to linear.}
    \label{projSpeed}
\end{figure}
...
ChordReduce is good because overhead scales logarithmically.
