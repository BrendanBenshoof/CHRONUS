...
Popular platforms for MapReduce, such as Hadoop \cite{Hadoop}, are explicitly designed to be used in large datacenters \cite{hadoopAssumptions}.  
\textit{The deployment of MapReduce at a data Center is the only option that has been heavily explored.}  
These MapReduce platforms are highly centralized and tend to have single points of failure\footnote{reference of Hadoop single point of failure and schedulers goes here} as a result.   A centralized design assumes that the network is relatively unchanging and does not usually have mechanisms to handle node failure during execution or, conversely, cannot speed up the execution of a job by adding additional workers on the fly.  Finally, learning curve for deploying these systems and developing programs on them \textit{is annoying}.


% http://wiki.apache.org/hadoop/Virtual%20Hadoop  <---- look here for motivation.
\textit{A data center/HPC context is only one in a wide variety of possible distributed frameworks}.  \textit{We were motivated to start exploring a more abstract deployment for MapReduce, one that could be deployed in a much wider variety of contexts. The is no reason that MapReduce jobs couldn't be run over peer-2-peer networks or as part of a volunteer computing framework.  No reason we can't use it in a data center either.}
\textit{A more abstract approach removes a lot of the assumptions:  failed nodes recover, network is static, failure is hardware based, powerful infrastructure}
\textit{We also wanted to create something much easier to deploy and program on.}\footnote{Expand on motivation more;  make reader feel that yes, this does seem like a good project to spend time on}.  
...
%Two parts: HDFS and Mapreduce/  Hadoop is intended for thousands of nodes on commodity hardware citation needed, but it's from the hortonwork vid I watched, basically saying scale out not up, as scaling out gives data + storage scaling, but this is because analysis is moved to data.   Key is avoiding random seeks.

%Name node keeps track of each block (64MB by default).  Can be rack aware. Logical file system; blocks are not necessarily and almost definitely not continuous. 

%Fault tolerance of DataNodes:  The DataNodes ping the NameNode.  If the NameNode doesn't get the ping, it removes the DataNode and that is replaced by  a replication

%MapReduce jobs are submitted by a client to the Jobtracker (in our framework it would be any node), gets split into tasks, which get distributed to the TaskTracker programs running on local data nodes who have the data.  Map occurs and output is sent over  network to reducer via shuffle/sort (which matchs up all the values with the same key) and sends that to the reducer.

%The MapReduce framewoork and HDFS component of Hadoop are codesigned/developed/deployed, so you have combined processing and storing data.  MapReduce wants to move processing to data and minimize the movement of data, since that is where most of the cost is.  \textbf{THe MapReduce component takes advantage of the fact that the data is replicated in the network}

%One thing to take from this is that the for the wordcount program, the pieces should/can already be stored across the network, most likely using CFS.

%MapReduce \textit{always} thinks of data in terms of keys and values.
...
The design of a distributed file system is closely tied to the design to the implementation of MapReduce.  see \cite{gfs} \cite{hdfs}
