...
Google's MapReduce \cite{mapreduce} paradigm has rapidly become an integral part in the world of data processing and is capable of efficiently executing numerous Big Data programming and data-reduction tasks.  By using MapReduce, a user can take a large problem, split it into small, equivalent tasks and send those tasks to other processors for computation.  The results are sent back to the user and combined into one answer.  \textit{There are two reasons this is awesome.  First, the MapReduce paradigms shifts the }
%Many popular platforms for MapReduce, such as Hadoop \cite{Hadoop}, are designed with data centers in mind, with low-lanency and high-width connections between multiple servers.  
...
The most popular platform for MapReduce is Hadoop \cite{Hadoop}. Hadoop is an open-source Java implementation deveolped by Apache and Yahoo! \cite{pavlo2009comparison}.  Hadoop has two components, the Hadoop Distributed File System (HDFS) and the Hadoop MapReduce Framework \cite{mrsurvey} \cite{shvachko2010hadoop}.  Under HFDS, nodes are arranged in a heirarchical tree, with a master node, called the NameNode, at the top.  The NameNode's job is to organize and distribute information to the slave nodes, called DataNodes.  This makes the NameNode a single point of failure \cite{shvachko2010hadoop} in the network, as well as a potential bottleneck for the system \cite{hadoop-bottle}.
...
Two parts: HFDS and MapreduceHadoop is intended for thousands of nodes on commodity hardware (citation needed, but it's from the hortonwork vid IO watched, basically saying scale out not up, as scaling out gives data + storage scaling, but this is because analysis is moved to data.   Key is avoiding random seeks)  Name node keeps track of each block (64MB by default).  Can be rack aware. Logical file system; blocks not necessarily and almost definitely not continuous. 

Fault tolerance of data nodes.  The DataNodes ping the NameNode.  If the NameNode doesn't  get the ping, it removes the data node and that is replaced by  a replication

Mapreduce jobs are submitted by a client to the Jobtracker (in our framework it would be any NameNode), gets split into tasks, whcih get distributed to the TaskTrackers on local data nodes that have the data.  Map occurs and output is sent over  network to reducer via shuffle/sort (which matchs up all the values with the same key) and sends that to the reducer.
