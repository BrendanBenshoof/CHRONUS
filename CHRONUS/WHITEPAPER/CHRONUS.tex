...
\begin{figure}
    ...
    \caption{A Chord network with 16 nodes and.}
    \label{chordreal}
\end{figure}
...
Since our data showed that approximating $\pi$ on one node took approximately 7 minutes, collecting ten times as many samples on a single node would take 70 minutes at minimum.  We also see that the local minima is pushed further to the right.

The 100 million sample set confirms that the network overhead is logarithmic.  At that job size, the job is too small to be effectively parallelized and we start seeing overheard acting as the dominant factor in out dataset.  This matches the behavior predicted by our equation, $time_{n} = \frac{time_{1}}{n} + k \cdot \log_{2}(n)$. For a small $time_{1}$, $\frac{time_{1}}{n}$  approaches 0 as $n$ gets larger, while $k \cdot \log_{2}(n)$, our overhead.  The samples from our dataset fit the curve $k \cdot \log_{2}(n)$, establishing that our overhead is increases logarithmicly with the number of workers.

Since we have now established that $time_{n} = \frac{time_{1}}{n} + k \cdot \log_{2}(n)$, we can approximate the mean of $k$ from our datapoints.  Using $k = $, we can estimate how long a job that takes 2 hours for a single node would take in parallel for abitrary number of nodes.  The execution time is shown in Figure and the corresponsing speedup in Figure .
