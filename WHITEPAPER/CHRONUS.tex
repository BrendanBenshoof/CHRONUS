...
%Two parts: HFDS and Mapreduce/  Hadoop is intended for thousands of nodes on commodity hardware citation needed, but it's from the hortonwork vid I watched, basically saying scale out not up, as scaling out gives data + storage scaling, but this is because analysis is moved to data.   Key is avoiding random seeks.
...
%The MapReduce framewoork and HFDS component of Hadoop are codesigned/developed/deployed, so you have combined processing and storing data.  MapReduce wants to move processing to data and minimize the movement of data, since that is where most of the cost is.  \textbf{THe MapReduce component takes advantage of the fact that the data is replicated in the network}
