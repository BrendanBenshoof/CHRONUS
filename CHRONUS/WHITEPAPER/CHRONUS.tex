...
One of the most popular platforms is Hadoop, Apache's open-source implementation of MapReduce \cite{Hadoop}.

CONNECTING SENTENCE(S) TO BRIDGE THE GAP

Chord \cite{Chord} is a distributed hash table (DHT) designed for peer-to-peer networks with a number of remarkable qualities.  Chord guarantees a worst-case $\log n$ lookup time for a particular node or file in the network. It is highly fault-tolerant to node failures and churn.  It scales extremely well and there is minimal maintenance required by the network as a whole to handle individual nodes.
 
However, the architectures that provide the backbone of many a peer-to-peer network, such as Chord \cite{Chord} have been viewed solely as a means for sharing files  or routing traffic.  We have developed a system, called CHRONUS, that takes the idea of evenly distributing files and applied to distributing work among member nodes.  We built a framework on CHRONUS for performing Map and Reduce tasks on Chord ring.  CHRONUS leverages the underlying protocol to distribute map and reduce tasks to nodes evenly, provide greater data redundancy, and guarantee a greater amount of fault tolerance.  The network has no single point of failure.

At the same time we avoid the architectural and file system constraints of systems like Hadoop.  Nodes in CHRONUS can be setup in a cluster for high performance or over the Internet, for volunteer computing tasks. 

Our experiments demonstrate that our framework is highly scalable, solving problems significantly faster when distributed.  The larger the problem is, the greater the speedup gained incorperating by incorperating more nodes into the problem. 


In section $number$ we discuss the Chord Protocol, as well as the specifics of our implementation.  We describe MapReduce and Hadoop in more detail in section $number$.  CHRONUS is described in Section  $X$, while our experiments and their results are covered in section $Y$.  Lastly, Section $number$ discusses fruitful avenues of future research.
...
THIS SECTION NEEDS OOMPH due to narrowing of focus.
