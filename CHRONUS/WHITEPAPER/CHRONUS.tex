...
One of the most popular platforms for MapReduce is Hadoop

CONNECTING SENTENCE(S) TO BRIDGE THE GAP

Chord \cite{Chord} is a distributed hash table (DHT) designed for peer-to-peer networks with a number of remarkable qualities.  Chord guaratees a worst-case $\log n$ lookup time for a particular node or file in the network. It is highly fault-tolerant to node failures and churn.  It scales extremely well and there is minimal maintenence required by the network as a whole to handle individual nodes.
 

However, the architectures that provide the backbone of many a P2P network, such as Chord \cite{Chord} have been viewed solely as a means for sharing files  or routing traffic.  We have developed a system, called CHRONUS, that takes the idea of evenly distributing files and applyed to distributing work among member nodes.  CHRONUS provides a scalable of evenly distributing both data and work among its member nodes.  Our experiments demonstrate that using  .  The larger the problem is, the greater the advantage of distributing the work and the gains of further distributing the work are increased (REWRITE). 

CHRONUS provides a number of useful benefits over Hadoop.  Chord's routing properties allows us to avoid the single point of failure that Hadoop has.
