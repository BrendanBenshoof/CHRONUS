...
%Two parts: HFDS and Mapreduce/  Hadoop is intended for thousands of nodes on commodity hardware (citation needed, but it's from the hortonwork vid IO watched, basically saying scale out not up, as scaling out gives data + storage scaling, but this is because analysis is moved to data.   Key is avoiding random seeks)  Name node keeps track of each block (64MB by default).  Can be rack aware. Logical file system; blocks not necessarily and almost definitely not continuous. 
...
The configurer does not actively participate in computing or distributing the work.  It exists to avoid having to purchase new instances each time we  shut down nodes to change experimental conditions. tell nodes to stop running experiments, 
