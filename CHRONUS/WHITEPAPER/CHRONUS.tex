...
$$time = \frac{time_{solo}}{n} + k \cdot \log_{2}(n)$$
$$speedup = \frac{time_{solo}}{\frac{time_{solo}}{n} + k \cdot \log_{2}(n)}$$ 
...
\subsection{Setup}
We ran our experiments using Amazons's EC2 service \footnote{I want to blaze through this section, adding citations later}.  %Initially, each node was a micro instance, but the specifications and the cpu burst behavior made the  
Each node was an individual EC2 small instance.  Once started, nodes pull the latest version of the code and run it, automatically joining the network.  We can choose any arbitrary node as the stager and tell it to run the MapReduce process. We found that the network was robust enough that we could take a node we wanted to be the stager out of the network, modify its MapReduce test code, have it rejoin the network, and then run the code new code without any problems. Since only the stager has to know how to create the map tasks, the other nodes don't have to be updated and are happy to perform the tasks they are given.

Once all nodes have booted, we run our Monte-Carlo approximation of $pi$.  Our experiments looked at the number of samples that were to be taken and the number of nodes performing MapReduce.


\subsection{Results}
Our experiments show that for a given problem, ChordReduce can effectively parralelize the problem, yielding a substansial speedup.  Furthermore, our results showed that the larger the problem is %what happens

Since our data showed that approximating $\pi$ on one node took approximately 7 minutes, collecting ten times as many samples on a single node would take an hour at minimum. 
