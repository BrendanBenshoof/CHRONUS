...
When a node $n$ changes his successor, he asks if the successor is holding any data $n$ should be responsible for.  The successor looks at all the data $n$ is better suited to hold into, packages it up, and sends it along to $n$.  The successor does not have to delete this data. If fact, keeping this data as a backup is benefitial to the network as a whole. 
...
The advantage of chunking allows for different options such as FUNKY FOUNTAIN OPERATIONS.  However, using an automated file replication technique such as IRM \cite{IRM} IS INCOMPATIBLE.
