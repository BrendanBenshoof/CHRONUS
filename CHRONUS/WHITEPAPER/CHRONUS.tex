...
We ran our experiments using Amazons's EC2 service \footnote{I want to blaze through this section, adding citations later}.  Each node was an individual EC2 small instance with a preconfigured Ubuntu 12.04 image.  Once started, nodes pull the latest version of the code and run it as a service, automatically joining the network.  We can choose any arbitrary node as the stager and tell it to run the MapReduce process. We found that the network was robust enough that we could take a node we wanted to be the stager out of the network, modify its MapReduce test code, have it rejoin the network, and then run the code new code without any problems. Since only the stager has to know how to create the map tasks, the other nodes don't have to be updated and are happy to perform the tasks they are given.

Our experiments were ran on groups of 1, 10, 20, 30, and 40 nodes.  Each sized ring was asked to generate 1 billion samples by default, with later tests done on 100 million and 10 billion.
...
Our experiments show that for a given problem, ChordReduce can effectively parralelize the problem, yielding a substansial speedup.  Furthermore, our results showed that the larger the problem is, the more workers could be added before dimishing returns were incurred.

Our default series was the 1 billion samples series.  On average, it took a single node 422 seconds, or approximately 7 minutes, to generate 1 billion samples.  Generating the same number of points in parallel with 10, 20, 30, or 40 nodes was always quicker.  Notice that the samples were generated fastest when there were 30 workers,

Since our data showed that approximating $\pi$ on one node took approximately 7 minutes, collecting ten times as many samples on a single node would take 70 minutes at minimum.  
