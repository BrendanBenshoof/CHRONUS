...
We implemented two ways of storing a file.  The first is the NORMAL WAY.  The second approach, forwarded by Dabek ETAL \cite{CFS}, is to break the files into chunks.  Each chunk gets its own hash value and gets stored in the corresponding location.  To find these disparate  chunks, nodes retreive a \emph{key block}, which is stored at the location corresponding to the hash of the whole file.  This key value contains 

The chunking allows for numerous advantages.  Each piece of the overall file would (ideally) be stored in a different node, each with a different backup.  This would prevent any single node from becoming overwhelmed from fulfilling multiple requests for a large file.  It would also prevent retrieval from being bottlenecked by a node with a relatively low bandwidth. 

Chunking also opens up the options for implementing additional redundancy such as erasure codes\cite{rizzo1997effective}, where redundant chunks are created, but any combination of a particular number of chunks is sufficient to recreate the file..  

However, using an automated file replication technique such as IRM \cite{IRM} HAS LIMITTED EFFECTIVENESS?.
...
\subsubsection{Handle nodes quitting while a job is running, politely or otherwise}
